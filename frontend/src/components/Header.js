...
import { ArrowUpRight, LogIn, LogOut, ShieldCheck } from "lucide-react";
...
export function Header({ theme, onToggleTheme }) {
  ...
  const visibleNav = user ? navItems : publicNav;
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-3 flex items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          {user ? (
            ...
            <>
              <span className="hidden lg:flex text-[10px] font-mono uppercase tracking-[0.22em] text-muted items-center gap-1.5 border border-border rounded-full px-2.5 py-1">
                modo demo
              </span>
              <NavLink
                to="/login"
                data-testid="header-entrar"
                className="text-[11px] font-mono text-accent hover:text-accentHover transition-colors flex items-center gap-1 border border-accent/40 rounded-md px-2.5 py-1 hover:bg-accentDim"
              >
